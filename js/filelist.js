				...
				$.ajax({
					...
					success: function (res) {
						...
						if (!data){
							debugger;
							if (self._filter) {
								hidden = fileData.name.toLowerCase().indexOf(self._filter.toLowerCase()) === -1;
							} else {
								hidden = false;
							}
							tr = self._renderRow(fileData, {updateSummary: false, silent: true, hidden: hidden});
							self.$fileList.append(tr);
							if (isAllSelected || self._selectedFiles[fileData.id]) {
